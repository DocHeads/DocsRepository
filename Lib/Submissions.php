...
class Submission
{
  ...
  public static function uploadFile()
  {
    ...
    if (move_uploaded_file($_FILES['submissionfile']['tmp_name'], $submissionUrl))
    {
      ...
      if (count($result) >= 1)
      {
        ...
        $isCommit = $this -> updateSubmission($email, $submissionUrl, $deptId, $courseId, $gradingUrl, $studentInstUrl, $instructorInstUrl, $comments);
      ...
      {
        $conn = new MySqlConnect();
        $insertSql = "INSERT INTO Submissions (docName, emailAddress, deptId, courseId, rubricFileName, studentInstructions, instructorInstructions, comments, createdDate, updateDate)";
        $insertSql .= "                VALUES ('{$submissionUrl}', '{$email}', '{$deptId}', '{$courseId}', '{$gradingUrl}, '{$studentInstUrl}', '{$instructorInstUrl}', '{$comments}', '{$ts}', '{$ts}')";
  ...
  public static function updateSubmission($email, $submissionUrl, $deptId, $courseId, $gradingUrl, $studentInstUrl, $instructorInstUrl, $comments)
  {
    $isCommit = FALSE;
    $conn = new MySqlConnect();
    $ts = $conn -> getCurrentTs();
    ...
    $updateSql .= "   SET emailAddress = '{$email}',";
    if (isset($gradingUrl))
    {
  ...
  /**
   * Method used to return the result set of submission records associated to a user.
