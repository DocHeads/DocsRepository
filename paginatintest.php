...
// You have to put your mysql connection data and alter the SQL queries(both
// queries)
...
if (isset($_GET['pn']))
{
  ...
  //$pn = ereg_replace("[^0-9]", "", $_GET['pn']); // filter everything but
  // numbers for security(deprecated)
}
else
{
...
// Be sure URL variable $pn(page number) is no lower than page 1 and no higher
// than $lastpage
if ($pn < 1)
{
...
// This line sets the "LIMIT" range... the 2 values we place to choose a range of
// rows from database in our query
$limit = 'LIMIT ' . ($pn - 1) * $itemsPerPage . ',' . $itemsPerPage;
// Now we are going to run the same query as above but this time add $limit onto
// the end of the SQL syntax
...
// This code runs only if the last page variable is ot equal to 1, if it is only
// 1 page we require no paginated links to display
if ($lastPage != "1")
{
...
<body>
   <div style="margin-left:64px; margin-right:64px;">
     <h2>Total Items: <?php echo $nr; ?><

/h2>
   </div>
      <div style="margin-left:58px; margin-right:58px; padding:6px; background-color:#FFF; border:#999 1px solid;"><?php echo $paginationDisplay; ?><
/div>
<div style="margin-left:64px; margin-right:64px;"><?php print "$outputList"; ?><
/div>
<div style="margin-left:58px; margin-right:58px; padding:6px; background-color:#FFF; border:#999 1px solid;"><?php echo $paginationDisplay; ?><
/div>
</body>
