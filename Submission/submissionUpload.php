...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
  ...
  if (!file_exists("{$fileUploadBaseDir}/{$dept}/{$submissionFile}"))
  {
    if (move_uploaded_file($_FILES['submissionfile']['tmp_name'], "{$fileUploadBaseDir}\\{$dept}\\{$submissionFile}"))
    {
      ...
      $conn -> freeConnection();
      // if inserted retrieve the record subID, and update it with the edit
      // button
      if ($isCommit)
      {
        $subID = '';
        $editButton = '';
        $conn = new MySqlConnect();
        $result = $conn -> executeQueryResult("SELECT subID FROM submissions WHERE docName = '{$docName}' AND emailAddress = '{$email}' AND createDate = '{$ts}'");
        
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC))
        {
          $subID = $row['subID'];
          // get the subID from the result set
          $editButton = '<a href="../Submission/submissionProfile.php?subID=' . $subID . '"><img width="13px" src="../Images/edit.png"></a>';
        }
        $conn -> freeConnection();
        // update the record just submitted
        $conn = new MySqlConnect();
        $isUpdated = $conn -> executeQuery("UPDATE submissions SET edit = '{$editButton}' WHERE subID = '{$subID}'");
        $conn -> freeConnection();
      }
    }
    else
